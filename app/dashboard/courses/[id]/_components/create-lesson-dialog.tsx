...
export function CreateLessonDialog({
  ...
  const [type, setType] = useState<"video" | "link" | "quiz">("video");

  const [state, action, pending] = useActionState<CreateLessonState, FormData>(
    createLesson,
    undefined
  );
  ...
  // Resetea el tipo al abrir el dialog
  useEffect(() => {
    if (open) setType("video");
  }, [open]);
        ...
        <form action={action} className="flex flex-col gap-4 pt-2">
          ...
          {type === "video" && (
            <div className="flex flex-col gap-1.5">
              ...
              <Select name="video_id" disabled={pending}>
                <SelectTrigger id="lesson-video" className="w-full">
          ...
          {type === "quiz" && (
            <p className="text-xs text-[var(--color-neutral-400)] bg-[var(--color-neutral-50)] rounded-lg px-3 py-2.5">
              Las preguntas y configuración del quiz se gestionan desde el menú de opciones de la lección una vez creada.
            </p>
          )}

          <Button type="submit" disabled={pending} className="text-white w-full mt-1">
            {pending ? "Creando..." : "Crear lección"}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
